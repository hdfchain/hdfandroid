...
package com.hdfandroid.dialog.send
...
import com.hdfandroid.view.util.AccountCustomSpinner
import com.hdfandroid.view.util.SCAN_QR_REQUEST_CODE
import hdflibwallet.Hdflibwallet
import hdflibwallet.TxAuthor
import hdflibwallet.TxFeeAndSize
...
class SendDialog(val fragmentActivity: FragmentActivity, dismissListener: DialogInterface.OnDismissListener) :
    ...
    private fun authorTx(): AuthoredTxData {
        ...
        var amountAtom = when {
            sendMax -> 0
            else -> Hdflibwallet.amountAtom(amount!!.toDouble())
        }
        ...
        if (amountHelper.exchangeDecimal == null) {
            ...
            val feeCoin = Hdflibwallet.amountCoin(feeAtom)
            val totalCostCoin = Hdflibwallet.amountCoin(totalCostAtom)
