...
package com.hdfandroid.fragments
...
class TransactionsFragment : BaseFragment(), AdapterView.OnItemSelectedListener, ViewTreeObserver.OnScrollChangedListener {
    ...
    private var txFilter = Hdflibwallet.TxFilterAll
    ...
    private fun refreshAvailableTxType() = GlobalScope.launch(Dispatchers.Default) {
        ...
        val txCount = wallet!!.countTransactions(Hdflibwallet.TxFilterAll)
        val sentTxCount = wallet!!.countTransactions(Hdflibwallet.TxFilterSent)
        val receivedTxCount = wallet!!.countTransactions(Hdflibwallet.TxFilterReceived)
        val transferredTxCount = wallet!!.countTransactions(Hdflibwallet.TxFilterTransferred)
        val stakingTxCount = wallet!!.countTransactions(Hdflibwallet.TxFilterStaking)
        val coinbaseTxCount = wallet!!.countTransactions(Hdflibwallet.TxFilterCoinBase)
    ...
    override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
        ...
        if (parent!!.id == R.id.timestamp_sort_spinner) {
            ...
            txFilter = when (position) {
                0 -> Hdflibwallet.TxFilterAll
                1 -> Hdflibwallet.TxFilterSent
                2 -> Hdflibwallet.TxFilterReceived
                3 -> Hdflibwallet.TxFilterTransferred
                else -> Hdflibwallet.TxFilterStaking
            }
